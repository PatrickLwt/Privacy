...
PARAMS = {
    'mnist': {'noise_type': 'Optimized_Special', 'epsilon': 5.0, 'delta': 0.00001, 'clip_value': 0.0001,
    ...
    'adult': {'noise_type': 'MM', 'epsilon': 10.0, 'delta': 0.00001, 'clip_value': 1.0,
              'train_epochs': 60, 'batch_size': 256, 'lot_size': 256, 'total_num_examples': 32561, 'load_model': False,
              'load_path': 'pretrain_model/pretrained_adult.h5',
...
net_info = {
    ...
    # Gaussian, UDN, Optimized_Special, Optimized_Normal, MVG, MM(Special for Adult)
    'noise_type': PARAMS[dataset_name]['noise_type'],
    ...
    # Whether to load pretrain model
    'load_model': PARAMS[dataset_name]['load_model'],
    'load_path': PARAMS[dataset_name]['load_path'],
    'net_name': PARAMS[dataset_name]['net_name'],
    # Whether to do data augmentation
    'augmentation': PARAMS[dataset_name]['augmentation'],
...
class OurModel(globals()[net_info['net_name']]):
    ...
    def train_step(self, data):
        ...
        noised_gradients = tf.nest.map_structure(
            self.reduce_noise_normalize_batch, clipped_gradients)

        for g, new_grad in zip(self.accumulated_grads, noised_gradients):
            g.assign_add(new_grad)

        tf.cond(self.apply_flag, lambda: self.apply(),
                lambda: self.not_apply())
...
def main(net_info):
    ...
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
        net_info['init_lr'], net_info['lr_decay_step'], net_info['lr_decay_rate'], staircase=True)
    ...
    if net_info['load_model']:
        ...
        for layer, initial in zip(model.layers, initial_weights):
            ...
            if weights and all(tf.nest.map_structure(np.array_equal, weights, initial)):
                print(
                    f'Checkpoint contained no weights for layer {layer.name}!')
            else:
                print(f'Loading weights for layer {layer.name}!')

    model.compile(optimizer=optimizer, loss=loss_custom,
